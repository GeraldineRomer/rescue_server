...
class UserController extends Controller
{
    ...
    public function destroy(User $user)
    {
        try {
            $user->delete();
            return response()->json(['message' => __('messages.deleted', ['Model' => __('user')])], 200);
        } catch (\Exception $e) {
            return response()->json(['message' => __('messages.cannot_delete_user')], 400);
        }
    }
    ...
    {
        $institution_id = $request->query("institution");
        /** @var User */
        $user = Auth::user();
